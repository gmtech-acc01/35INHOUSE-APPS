...
use App\Workers\SMS_BULKY\SMSBulkyWorker;
...
class SMSBulkyListener extends Listener{
	
    public function __construct() {	
    	$this->load_env();
    	echo "*** PHP R-MQ SMS RECEIVER (BULKY) ***\n\n";
    	echo "--".getenv('Q_NAME')."--\n";
    }

    private function getSysRefToken($length = 18){
    ...
    private function totalSMSCountInBody($body){
    ...
    public function check_inputs($data){
    	//var_dump($data) ;
    	$truth = true;
    	if(!isset($data['recepients'])) $truth = false;
...
//units_consumed
    	try{
	    	...
	    	$smsOutgoing->status = 'BULKY';//$module_results["response_type"];
	    	$smsOutgoing->response_type = 'BULKY';//$module_results["response_type"];
	    	$smsOutgoing->response_info = 'BULKY';//$module_results["response_info"];
	    	$smsOutgoing->error_info = 'BULKY';//$module_results["error_info"];
	    	$smsOutgoing->update();

	    	//check if not sent increase the quota(refund customer)
	    	
	    	if($module_results["response_type"] != 'SENT'){
	    		//$customer_id add sms_quota to a customer _id
	    		$acc = CustomerAccount::find($customer_id);
	    		if($acc == null) return;
	    		//$acc->sms_quota = $acc->sms_quota + ($module_results["units_consumed"]);
	    		//$acc->update();
	    		//echo "NOT-SENT";
	    	}
    ...
    */ 
    public function process(AMQPMessage $msg)
    {
		...
		if($oldLog != null){
	    	...
	    	$log_entry->batch_id = isset($MSG_OBJ['batch_id']) ? $MSG_OBJ['batch_id'] : '';
	    	$log_entry->batch_no = isset($MSG_OBJ['batch_no']) ? $MSG_OBJ['batch_no'] : '';
	    	...
	    	$log_entry->json_receivers = json_encode($MSG_OBJ['recepients'],true);
	    	$log_entry->sender_name = $account == null ? '' : $account->api_sender_name;
	    	$log_entry->sms_text_length = strlen($MSG_OBJ['message']);
	    	$log_entry->sms = $MSG_OBJ['message']; 
	    	$log_entry->sms_counts = $this->totalSMSCountInBody($MSG_OBJ['message']) * (sizeof($MSG_OBJ['recepients']));
		...
		/*send sms*/
		$worker = new SMSBulkyWorker(); 
		$module_results = $worker->send_bulky_sms($MSG_OBJ['account_no'],$MSG_OBJ['recepients'],$MSG_OBJ['message'],$_sender_name);
		...
		$smsBatch = SMSLogSchBatch::where('batch_id',$MSG_OBJ['sys_ref'])->update([
			'sent_units'=>$module_results["units_consumed"],
			'response_type'=>$module_results["response_type"],
			'response_info'=>$module_results["response_info"],
			'error_info'=>$module_results["error_info"],
...
/*START LISTENING*/
$listener = new SMSBulkyListener;//$listener->log('shutdown','email.txt');
try{
