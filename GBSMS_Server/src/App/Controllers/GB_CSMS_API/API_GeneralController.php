	...
	*/
	public function send_sms($req,$res,$args){
		...
		$acc_status = $this->checkAccountStatus($account,$accountInput['api_key']);
 		...
 		/*Sender name check*/
 		$sms_sender_name = ($sms_sender_name =="")?"INFO":$sms_sender_name;
 		$account_sender_names = json_decode($account->sender_names,true);
 		$sender_name_found = false;
 		for($i = 0; $i <sizeof($account_sender_names);$i++){
 			if($sms_sender_name == $account_sender_names[$i]){
 				$sender_name_found = true;
 				break;
 			}
 		}
 		if($sender_name_found == false){
 			return $res->withJson([
				"code"=>4006,
				"code_status" => "Invalid Sender Name.",
				"msg_status" => "ERROR"
			])->withStatus(200);
 		}
